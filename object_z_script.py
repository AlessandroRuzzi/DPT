...
for kid in kid_list:
  ...
  pred_obj_z = ((torch.mean(img_tensor[x_pers_pos[0]: x_pers_pos[1],y_pers_pos[0]: y_pers_pos[1]])) * (torch.min(verts[0,:,2]) + (torch.max(verts[0,:,2]) - torch.min(verts[0,:,2])) / 2.0)) / (torch.mean(img_tensor[object_center[2][0]:object_center[2][2],object_center[2][1]:object_center[2][3]]))
